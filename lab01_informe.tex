...
En la página de inicio de sesión (\texttt{login.php}) se repite la misma lógica: el formulario que envía la acción \texttt{/questions/login.php} incluye los campos \texttt{vb\_login\_md5password} y \texttt{vb\_login\_md5password\_utf}, ambos rellenados por \texttt{md5hash()} justo antes del envío. El archivo \texttt{md5-password-analysis.json} confirma la presencia de las funciones \texttt{hex\_md5()}, \texttt{core\_md5()} y sus auxiliares, demostrando que la contraseña del usuario se convierte en MD5 hexadecimal antes de transmitirse. Durante las pruebas manuales, el valor generado por el navegador para ``dwQzwk9*yW2QnLYPYhB\$KS\$\#ADQ'' coincidió con el digest obtenido en consola, evidenciando la ausencia de sal o mecanismos adicionales.
