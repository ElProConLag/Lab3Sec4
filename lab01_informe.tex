...
El sitio enlaza su política en \url{https://www.linuxquestions.org/linux/privacy.html}. Tras revisar el documento completo, se observa que la política establece lo siguiente:

\begin{itemize}
  \item Declara recopilar información personal (dirección de correo, nombre de usuario, contraseña e IP).
  \item Afirma no vender información a terceros y limitar el uso de datos al propósito inicial.
  \item Menciona genéricamente que posee ``medidas de seguridad'' para proteger la información, sin especificar cuáles.
\end{itemize}

Sin embargo, la política presenta omisiones críticas respecto a la seguridad de contraseñas:

\begin{itemize}
  \item No menciona el algoritmo de hash empleado (MD5, bcrypt, Argon2, etc.), ni la presencia o ausencia de sal.
  \item No detalla políticas de expiración de sesiones, bloqueo por intentos fallidos, o limitación de velocidad (rate limiting).
  \item No alude a mecanismos de autenticación multifactor (MFA) o verificación adicional.
  \item No especifica cifrado en tránsito (TLS/HTTPS obligatorio) o almacenamiento seguro del lado servidor.
  \item Omite completamente procedimientos de recuperación de cuenta, cambio de contraseña o revocación de sesiones.
  \item No aborda auditoría, registros (logging) de acceso fallido, o notificaciones de incidente.
\end{itemize}

La política es genérica y no proporciona transparencia respecto a los controles específicos de seguridad de credenciales, lo que genera confianza infundada en los usuarios y refleja un enfoque reactivo frente a un modelo proactivo de seguridad moderna.
